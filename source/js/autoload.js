...
const live2d_path = window.location.origin + '/js/live2d-widget/';
...
(async () => {
  ...
  // Load smart greeting system first
  // 先加载智能问候系统
  await loadExternalResource(live2d_path + 'smart-greeting.js', 'js');
  ...
  // Show personalized greeting after widget loads
  // 看板娘加载完成后显示个性化问候
  setTimeout(() => {
    if (window.smartGreeting) {
      const greeting = window.smartGreeting.getPersonalizedGreeting();
      // Dispatch custom event to show message
      window.dispatchEvent(new CustomEvent('live2d:greeting', { detail: greeting }));
    }
  }, 2000);
})();
